...
// Initialize the athletic font smoothly via Next.js Google Fonts
...
export default function Hero({ onRsvpClick }: HeroProps) {
  ...
  useEffect(() => {
    const runCinematicSequence = async () => {
      // Step 1: Initial logo entry slide-in
      await Promise.all([
      ...
      // Step 2: Dramatic logo collision flashes
      for (let i = 0; i < 3; i++) {
        await Promise.all([
      ...
      // Step 3: Locked Viewport flight anchors to prevent text collision
      const isMobile = window.innerWidth < 768;
      await Promise.all([
        leftLogoControls.start({ 
          x: isMobile ? "-33vw" : "-42vw", 
          y: isMobile ? "-43dvh" : "-40vh", 
      ...
      // Step 4: Reveal primary marketing copy text
  ...
  return (
    <section className="relative w-full min-h-[100dvh] overflow-x-hidden bg-slate-950 flex flex-col items-center justify-start md:justify-center pt-24 pb-16 md:py-0">
      
      {/* 1. Video Layer with Color Grading */}
      <video 
        autoPlay 
        loop 
        muted 
      ...
      {/* 3. Readability Backdrop Layer */}
      <div className="absolute inset-0 bg-slate-950/60 z-10 backdrop-blur-[1px]" />

      {/* 4. Isolated Viewport Logo Layer (Wired directly to dvh to guarantee accurate top alignment) */}
      <div className="absolute top-0 left-0 w-full h-[100dvh] z-20 flex items-center justify-center pointer-events-none">
      ...
      {/* 5. Main Typography & Conversion Layer */}
      <motion.div 
        initial={{ y: -20, opacity: 0 }} 
        animate={textControls} 
        className="relative z-30 flex flex-col items-center text-center px-4 max-w-4xl mt-6 md:mt-0"
      >
        <h2 className="text-[#1E90FF] font-bold tracking-widest uppercase mb-2 text-[10px] md:text-sm">Encompass Group Presents</h2>
        
        <h1 className={`${bebas.className} text-4xl md:text-7xl lg:text-8xl text-white tracking-wide mb-3 leading-none px-2`}>
          Business Owners Night at the <br />
          <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#1E90FF] to-[#00A650]">
            Hartford Yard Goats
          </span>
        </h1>
        
        <p className="text-[#00A650] font-semibold text-xs md:text-xl tracking-wide mb-4 mt-1">Build Relationships. Share Ideas. Enjoy the Game.</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-2 md:gap-6 border-y border-slate-800/60 py-3 w-full max-w-3xl mb-8 text-center text-xs md:text-sm text-slate-200 bg-slate-900/40 backdrop-blur-sm rounded-lg px-2">
          ...
          <div className="border-y md:border-y-0 md:border-x border-slate-800/60 py-2 md:py-0"><span className="text-[#1E90FF] font-bold block md:inline">LOCATION:</span> Private Pavilion at Dunkin' Park</div>
          <div><span className="text-[#00A650] font-bold block md:inline">ADMISSION:</span> Complimentary for You & Guest</div>
        </div>

        <button 
          onClick={onRsvpClick} 
          className="group relative px-8 py-4 bg-white text-slate-950 font-black uppercase tracking-wider text-xs md:text-sm rounded-sm overflow-hidden transition-all hover:scale-105 shadow-[0_0_20px_rgba(30,144,255,0.5)]"
        >
